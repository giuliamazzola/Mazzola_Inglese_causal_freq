...
This work is submitted together with the homonymous paper to the journal *Corpus Linguistics and Linguistic Theory* in December 2025.
...
![Dendogram and Scree plot Italian](Ita_VNC_plots.jpg)


![Dendogram and Screeplot Spanish](Es_VNC_plots.jpg)
...
```
The next candidate for removal is `s(causalness_lagged)` in *model6* (as we tried already removing `s(causalness_lagged, by = language, k = 10, bs = "cs", m = 1)`. It is not completely ok to remove that, because the anova is marginally significant and the AIC is lower for *model 5*. 

So we go back to *model 5* and this time in *model 7* we remove `s(causalness_lagged, by = language, k = 10, bs = "cs", m = 1)`. It seems that *model5* is our final model, as removing this group-wise smooth is not ok.
